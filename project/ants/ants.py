...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        number_distance_place = 0
        while current_place != beehive:
            if self.min_range<= number_distance_place <= self.max_range:
                if len(current_place.bees) != 0:
                    return rANTdom_else_none(current_place.bees) # REPLACE THIS LINE
                else:
                    current_place = current_place.entrance
                    number_distance_place += 1
            else:
                current_place = current_place.entrance
                number_distance_place += 1
        return None
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        """
        # BEGIN Problem 5
        copy_list_bees = list(self.place.bees)
        Ant.reduce_armor(self, amount)
        for bee in copy_list_bees:
            Insect.reduce_armor(bee, amount)
            if bee.armor >0 and self.armor <=0:
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 9
        return self.contained_ant is None and \
            not isinstance(other,ContainerAnt)
        # END Problem 9
...
# END Problem 13
    ...
    def action(self, gamestate):
        ...
        if self.is_impostor:
            ...
            while place_check is not None:
                if place_check.ant is not None and \
                not place_check.ant in self.double_damage_ants :
                    place_check.ant.damage *= 2
                    self.double_damage_ants.append(place_check.ant)
                if isinstance(place_check.ant, ContainerAnt) and \
                place_check.ant.contained_ant is not None and \
                not place_check.ant.contained_ant in self.double_damage_ants:
                    place_check.ant.contained_ant.damage *= 2
                    self.double_damage_ants.append(place_check.ant.contained_ant)
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor <= 0 and not self.is_impostor:
            bees_win()
        # END Problem 13
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        # BEGIN EC
        if self.scared and self.change_direction:
...
def apply_status(status, bee, length):
    ...
    # BEGIN Problem EC
    action_before_status = bee.action
    new_action = status(action_before_status, bee)
    def helper(gamestate):
        nonlocal length
        if length > 0:
            length -=1
            ...
            return action_before_status(gamestate)
    bee.action = helper
    # END Problem EC
