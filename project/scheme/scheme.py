...
def eval_all(expressions, env):
    ...
    return scheme_eval(expressions.first, env, True) # change this line
    # END PROBLEM 7
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        validate_formals(func_formals)
        env.bindings[func_name] = LambdaProcedure(func_formals, func_body, env)
        return func_name
    else:
...
def do_and_form(expressions, env):
    ...
    while expressions.rest is not nil:
        ...
        if is_false_primitive(evaluated) or expressions.rest is nil:
            return evaluated
        expressions = expressions.rest
...
def do_or_form(expressions, env):
    ...
    while expressions.rest is not nil:
        evaluated = scheme_eval(expressions.first, env)
        if is_true_primitive(evaluated) or expressions.rest is nil:
            return evaluated
        expressions = expressions.rest
    return scheme_eval(expressions.first, env, True)
    # END PROBLEM 12
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        each_binding = bindings.first
        #print(bindings.rest)
        validate_form(each_binding, 2, 2)
        names = Pair(each_binding.first, names)
        values = Pair(scheme_eval(each_binding.rest.first, env), values)
...
def do_define_macro(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        func_body = expressions.rest
        validate_formals(func_formals)
        env.bindings[func_name] = MacroProcedure(func_formals, func_body, env)
        return func_name
    else:
        bad_target = target.first if isinstance(target, Pair) else target
        raise SchemeError('non-symbol: {0}'.format(bad_target))
