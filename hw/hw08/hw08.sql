...
CREATE TABLE by_parent_height AS
  SELECT dog_child.name FROM dogs AS dog_child, parents, dogs AS dog_parent
  WHERE dog_child.name = child AND parent = dog_parent.name ORDER BY dog_parent.height DESC;
...
CREATE TABLE sentences AS
  ...
  WHERE p1.parent = p2.parent AND p1.child = s1.name AND p2.child = s2.name AND p1.child < p2.child AND s1.size = s2.size;
