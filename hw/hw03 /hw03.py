...
def composer(func=lambda x: x):
    ...
    """
    def func_adder(g):
        return composer(lambda y: func(g(y)))
...
def missing_digits(n):
    ...
    last_num = n % 10
    rest_num = n // 10
    return missing_digits(rest_num) + max((last_num - rest_num % 10) - 1, 0)
...
def count_change(total):
    ...
    True
    """
    def change_helper(total, smallest_coin):
        if total == 0:
            return 1
        elif smallest_coin > total:
            return 0
        without_smallest_coin = change_helper(total, smallest_coin * 2)
        with_smallest_coin = change_helper(total - smallest_coin, smallest_coin)
